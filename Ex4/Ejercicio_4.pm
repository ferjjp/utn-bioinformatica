...
GetOptions(
    'f=s' => \$input,
    'p=s' => \$pattern
);    # Leemos de consola el input file y el pattern.
...
while ( my $result = $input_file->next_result ) {
    while ( my $hit = $result->next_hit ) {    # Itera sobre el archivo blast.
        ...
        { # Checkea si el pattern esta contenido dentro de la description, si el index devuelve -1 es porque no se encuentra.
            my $acc         = $hit->accession;
            my $seq         = $db->get_Seq_by_acc($acc);
            my $file        = ">$acc.fas";
            my $output_file = Bio::SeqIO->new(
                -file => $file
                , # Genera un archivo FASTA por cada coincidencia que encuentre.
                -format => 'fasta',
            );
